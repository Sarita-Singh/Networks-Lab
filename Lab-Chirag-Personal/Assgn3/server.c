...
int main(int argc, char* argv[])
{
    ...
    if(argc <= 1) {
        printf("Please provide the server's port\n");
		exit(0);
    }
	...
	while (1) {
		...
		*/
        ...
        if(strcmp(buf, "Send Load") == 0) {
            ...
            snprintf(buf, sizeof(buf), "\a%s\n", ctime(&current_time));
            send(newsockfd, buf, strlen(buf)+1, 0);
        }
        else {
            printf("Cannot understand request. Please try again.");
            strcpy(buf,"Cannot understand request.");
            send(newsockfd, buf, strlen(buf) + 1, 0);
        }
